<?php
    ...
    class XMLActeReader {
        ...
        function read_personne_node($xml_personne){
            ...
            $attributes = $xml_personne->attributes();

            if(isset($attributes["id"]))
                $personne->id = $attributes["id"];

            if(isset($attributes["don"])
                && $attributes["don"] == "true")
            ...
            foreach($xml_personne->children() as $xml_child){
                switch($xml_child->getName()){
                    ...
                    case "nom":
                        ...
                        $attribute = $xml_child->attributes()["attr"];
                        if(!isset($attribute))
                            $attribute = NULL;
                        $personne->add_nom(new Nom(NULL, $xml_child->__toString(), NULL, $attribute));
                        break;
                    case "pere":
